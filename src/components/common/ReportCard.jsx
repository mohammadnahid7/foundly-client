	...
	return (
		<>
			<div
				...
				<div
					className={`${isListMode ? "w-32 md:w-40 rounded-l-xl" : "rounded-t-xl h-60"} shrink-0 dark:bg-pink-800 bg-no-repeat bg-cover`}
				...
				<div className={`${isListMode ? "flex-col items-left flex-1" : "flex-col"} p-4 h-full text-left flex relative`}>
					...
					<div className={`${isListMode ? "flex-1 w-full" : "w-full mt-5"}`}>
						<h2 className={`${isListMode || "mb-2"} font-medium text-gray-800 dark:text-neutral-300`}>{el?.title}</h2>
					...
					<dl
						...
						{isListMode || (
							<>
								<dt className="py-1 text-sm text-gray-500 dark:text-neutral-500">Type:</dt>
								<dd className="py-1 capitalize inline-flex justify-end items-center gap-x-2 text-end font-medium text-sm text-gray-800 dark:text-neutral-200">
									{el?.type}
								</dd>
							</>
						)}
						<dt className="py-1 text-sm text-gray-500 dark:text-neutral-500">Status:</dt>
						<dd className="py-1 capitalize inline-flex justify-end items-center gap-x-2 text-end font-medium text-sm text-gray-800 dark:text-neutral-200">
							{el?.status}
						</dd>
						<dt className="py-1 text-sm text-gray-500 dark:text-neutral-500">Location:</dt>
						<dd className="py-1 capitalize inline-flex justify-end items-center gap-x-2 text-end font-medium text-sm text-gray-800 dark:text-neutral-200">
							{el?.location}
						</dd>
						{isListMode || (
							<>
								<dt className="py-1 text-sm text-gray-500 dark:text-neutral-500 capitalize">{el?.type} Date:</dt>
					...
					<div
						...
						{el?.authormeta?.uid == user?.uid ? (
							...
							<button
								...
								{el?.type == "lost" ? "Found This!" : "This is Mine!"}
							</button>
						)}
